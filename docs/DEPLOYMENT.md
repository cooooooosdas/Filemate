...
> `v1.3.0-alpha.1` 已在生产模式下提供匿名设备级隔离：每个浏览器获得签名 HttpOnly Cookie，并使用独立 SQLite、上传目录和归档目录。它修复了不同访客互相看到数据的问题，但不等同于账号系统；清除 Cookie 或更换浏览器后不能自动找回原数据。

当前公开体验站只用于产品展示，已启用 HTTPS、Host/CORS 白名单、可信 Origin 校验、上传体积限制、安全响应头，并关闭公网模型密钥设置、内部关停接口和 API 文档。以上措施与匿名设备隔离减少了跨访客泄露面，但不能替代账号、配额、限流、审计和备份恢复。

生产环境保持 `FILEMATE_IDENTITY_MODE=anonymous`。身份签名密钥默认保存在持久数据卷的 `identity.secret`；也可显式配置至少 32 字符的 `FILEMATE_IDENTITY_SECRET`。不得在仍有活跃用户时随意更换该密钥，否则现有 Cookie 会失效。
...
2. 将匿名设备级分库升级为正式账户与租户授权，支持跨设备登录、身份找回和管理员停用。
