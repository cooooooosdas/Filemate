...
const data = ref<LearningAnalytics | null>(null); const loading = ref(true)
const format = (value:number) => Math.round(value || 0)
const load = async () => { loading.value=true; try{data.value=await getLearningAnalytics()}catch(error:any){ElMessage.error(error.message||'加载失败')}finally{loading.value=false} }
